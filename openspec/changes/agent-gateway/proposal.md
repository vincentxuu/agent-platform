...
- `agent-gateway-web-ui`: Defines the local-first Web UI surfaces for Run, Flows, Skills, Providers, Policies, Context, Memory, Runs, Evaluations, Observability, Evidence, and Artifacts.
...
- MVP scope: prioritize Deep Research, local skill packages, basic provider/tool routing, policy guards, checkpoint/resume, evidence-backed Markdown reports, JSON evidence bundles, timeline/context/evidence/artifact UI, and eval/learning capture without automatic production updates.
