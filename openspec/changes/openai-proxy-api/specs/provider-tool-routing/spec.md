...
- **THEN** the system persists the provider configuration and makes it available to flow validation, runtime routing, **and proxy model routing**
...
- **WHEN** an operator disables a provider used by existing flow versions **or referenced in proxy model mapping**
- **THEN** the system prevents new runs from selecting that provider **and excludes it from proxy model list** while preserving prior run audit records
...
#### Scenario: Select a provider for proxy request
- **WHEN** a proxy request specifies an OpenAI model ID mapped to one or more providers with fallback priorities
- **THEN** the system selects the highest-priority enabled and ready provider for that model
...
#### Scenario: Proxy provider fallback
- **WHEN** a proxy request's primary mapped provider returns an error, timeout, or exceeds budget/rate limits
- **THEN** the runtime tries the next priority provider for the same OpenAI model ID and records the failed provider, fallback provider, fallback reason, and outcome

#### Scenario: Proxy fallback loop prevention
- **WHEN** a proxy request has attempted 3 fallback providers
- **THEN** the runtime stops fallback attempts and returns an error
...
#### Scenario: Inspect proxy provider cost
- **WHEN** a user reviews proxy request observability
- **THEN** the system shows provider usage for proxy requests with model ID, provider ID, fallback count, tokens, cost, and latency